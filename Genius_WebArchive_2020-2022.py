...
start_date_formated = datetime.strptime("2020 01 01", '%Y %m %d')
...
data = []
for j in range(500):
    date = [new_start_date + timedelta(days=j) for j in range(500)]
...
print(lyrics_df)
lyrics_df.to_excel("lyrics_800_sorted.xlsx")
lyrics_df.columns = ["Lyrics URL", "Lyrics"]

results = pd.merge(data, lyrics_df, on="Lyrics URL")

results = pd.concat([data, lyrics_df], axis=1)
results.to_excel("title_lyrics_data.xlsx")
